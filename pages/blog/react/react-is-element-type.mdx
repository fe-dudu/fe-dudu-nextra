...
```
1. 먼저 object에 `$$typeof`를 체크하여 Element 요소인지, Portal을 사용하여 특정 태그를 DOM에 삽입하는지 체크한다.
2. `REACT_ELEMENT_TYPE`에 포함되는 `REACT_FRAGMENT_TYPE`, `REACT_PROFILER_TYPE`, `REACT_STRICT_MODE_TYPE`, `REACT_SUSPENSE_TYPE`, `REACT_SUSPENSE_LIST_TYPE`은 type 그대로 반환한다.
   - 여기서는 `$$typeof`이 아닌 `type`을 체크한다.
   - `type`은 symbol 값을 갖는다.
3. `type.$$typeof`(`type` 객체 내부에 `$$typeof` 값)을 확인하고 반환한다.
   - 여기서는 `$$typeof`이 아닌 `type.$$typeof`을 체크한다.
   - `type.$$typeof` 또한 symbol 값을 갖는다.


## 참고 자료
[React | react-is](https://github.com/facebook/react/blob/main/packages/react-is/src/__tests__/ReactIs-test.js)