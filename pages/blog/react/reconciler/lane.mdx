...
**우선순위**가 더 높은 업데이트를 먼저 처리하고, 불필요한 렌더링을 하는 것을 방지합니다. 직역해보면 어떤 우선순위 도로에 있는지 확인하는 작업입니다.
...
### `SyncLane`
- (NoLane을 제외하고) 가장 높은 우선순위를 가지며 사용자 상호작용(클릭, 입력 등)을 즉각 처리해야 할 때 사용됩니다.

### `SyncHydrationLane`
- 서버에서 전송된 HTML과 React의 상태를 동기화할 때 사용됩니다.

### `InputContinuousLane`
- (NoLane을 제외하고) 두 번째 우선순위를 가지며 지속적인 입력 이벤트(예: 키 입력, 스크롤)와 관련된 작업입니다.

### `InputContinuousHydrationLane`
- 입력 이벤트(예: 키 입력, 스크롤)와 관련된 서버-클라이언트 동기화를 처리합니다.

### `DefaultLane`
- (NoLane을 제외하고) 세 번째 우선순위를 가지며 리액트 외부(`fetch`, `setTimeout` 등)에서 발생한 이벤트를 처리합니다.
...
### `IdleLane`
- (NoLane을 제외하고) 네 번째 우선순위를 가지며 매우 낮은 우선순위의 작업으로 CPU가 유휴 상태일 때만 처리됩니다.
